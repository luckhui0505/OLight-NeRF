...
class NeRFMLP(nn.Module):
    def __init__(
        ...
        for idx in range(netdepth - 1):
            if idx % skip_layer == 0 and idx > 0:   # 在第四层的时候再次插入位置信息(x,y,z)
        ...
        self.pts_linears = nn.ModuleList(pts_linear)    # 体素密度预测部分网络的全部结构
...
class LitNeRF(LitModel):
    ...
    def test_epoch_end(self, outputs):
        ...
        if self.trainer.is_global_zero:
            ...
            #store_image.store_video(image_dir, rgbs)  # store video for multi-view
