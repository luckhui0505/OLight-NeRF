...
class NeRFMLP(nn.Module):
    def __init__(
        ...
        self.enhance_net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(32, 3),
            nn.Sigmoid()  # 输出在[0,1]之间的增强系数
        )

        self.occlusion_mlp = nn.Linear(netwidth_condition, 1)
    ...
    def forward(self, x, condition):
        ...
        raw_darkness = raw_darkness.reshape(
            -1, num_samples, self.num_density_channels
        ).permute(1, 2, 0)
        ...
        # 5. 拼接视角条件
        condition_tile = torch.tile(
            condition[:, None, :], (1, num_samples, 1)
        ).reshape(-1, condition.shape[-1])

        x_cond = torch.cat([bottleneck, condition_tile], dim=-1)

        # 6. 视角分支
        for idx in range(self.netdepth_condition):
            x_cond = self.views_linear[idx](x_cond)
            x_cond = self.net_activation(x_cond)

        # 7. RGB 输出：这里必须用 x_cond，不是 x
        raw_rgb = self.rgb_layer(x_cond).reshape(
            -1, num_samples, self.num_rgb_channels
        )

        # 8. 可学习遮挡权重
        raw_occlusion = self.occlusion_mlp(x_cond).reshape(
            -1, num_samples, 1
        )

        return raw_rgb, raw_density, raw_darkness, raw_occlusion
...
class Aleth_NeRF(nn.Module):
    def __init__(
        ...
        use_adaptive: bool = True,  # 新增：是否使用自适应增强
        occ_scale: float = 10.0,      # ✅ 新增
        alpha: float = 0.1,           # ✅ 新增
        warmup_steps: int = 5000, # ✅ 新增
    ):
        for name, value in vars().items():
            if name not in ["self", "__class__"]:
                setattr(self, name, value)
        ...
        ##################改#############g
        self.use_enhance = use_enhance  # 添加这行
        self.enhance_strength = enhance_strength
        self.use_adaptive = use_adaptive  # 新增
        self.occ_scale = occ_scale
        self.alpha = alpha
        self.warmup_steps = warmup_steps
        ############改################g
    def simple_enhance(self, rgb):
        """简单的增强：rgb_light = rgb + enhance_strength * rgb * (1 - rgb)"""
        if self.enhance_strength <= 0:
            return rgb
        
        enhancement = self.enhance_strength * rgb * (1 - rgb)
        rgb_light = rgb + enhancement
        return torch.clamp(rgb_light, 0, 1)

    def adaptive_enhance(self, rgb):
        """自适应增强：每个像素根据亮度动态调整增强强度"""
        if self.enhance_strength <= 0:
            return rgb
        ...
        luminance = luminance.unsqueeze(-1)  # [B, N, 1]
        
        # 2. 自适应规则：暗区域增强更多
        # 亮度越低，增强强度越大（范围：0.15-0.45）
        base_strength = self.enhance_strength  # 0.3
        
        # 暗度因子：暗区域增强更多（亮度<0.5的区域）
        # 公式：强度 = 0.3 * (1.0 + 0.5 * (0.5 - 亮度))
        dark_factor = 1.0 + 0.5 * (0.5 - luminance).clamp(min=0)
        
        # 3. 避免亮区域过度增强
        # 亮度>0.7的区域减少增强
        bright_factor = torch.where(
            luminance > 0.7,
            1.0 - 0.3 * (luminance - 0.7) / 0.3,  # 亮度0.7-1.0：因子1.0→0.7
            torch.ones_like(luminance)
        )
        
        # 4. 综合自适应因子
        adaptive_factor = dark_factor * bright_factor
        adaptive_strength = base_strength * adaptive_factor
        
        # 5. 应用增强
        mask = rgb * (1.0 - rgb)
        enhancement = adaptive_strength * mask
    ...
    def forward(self, rays, randomized, white_bkgd, near, far, mode,global_step=None): # white_bkgd: False; near: None; far: None
        ...
        for i_level in range(self.num_levels):
            ...
            raw_rgb, raw_sigma, raw_darkness, raw_occlusion = mlp(samples_enc, viewdirs_enc)
            ...
            # 应用增强（只在测试阶段）
            if mode == 'test' and self.enhance_strength > 0 and self.use_enhance:
                if self.use_adaptive:
                    rgb = self.adaptive_enhance(rgb)  # 使用自适应增强
                else:
                    rgb = self.simple_enhance(rgb)    # 使用原始增强
                    
     
            # Local Conceling Field 
            darkness = self.dark_activation(raw_darkness)   # nn.Sigmoid()
            # 关键修改 1：放大 raw_occlusion，避免 sigmoid 后塌在 0.5 附近
            #occ_scale = 10.0
            occlusion = torch.sigmoid(raw_occlusion * self.occ_scale)


            if mode == 'train' and global_step is not None:
                warmup_ratio = min(1.0, float(global_step) / self.warmup_steps)
            else:
                warmup_ratio = 1.0
            alpha = self.alpha * warmup_ratio

            # 关键修改 3：对调制项做 clamp，避免 sigma 被破坏太多
            modulation = 1.0 + alpha * (occlusion - 0.5)
            modulation = torch.clamp(modulation, 0.85, 1.15)

            sigma = sigma * modulation
            
            if self.training and torch.rand(1).item() < 0.001:
                print("occlusion mean:", occlusion.mean().item())
                print("occlusion std:", occlusion.std().item())
                modulation = 1.0 + alpha * (occlusion - 0.5)
                print("modulation min/max:", modulation.min().item(), modulation.max().item())            
            ...
            if mode != 'test':
                ...
                ret.append((comp_rgb_dark, darkness, sigma, depth, weights, comp_rgb,occlusion))
...
class LitAleth_NeRF(LitModel):
    def __init__(
        ...
        use_enhance: bool = True,
        enhance_strength: float = 0.0,
        enhance_weight: float = 0.01,  
        use_adaptive: bool = True,  # 新增
        occ_scale: float = 10.0,
        alpha: float = 0.1,
        ...
        ###################################33
        self.model = Aleth_NeRF(
            enhance_strength=enhance_strength,
            use_adaptive=use_adaptive,  # 传递参数
            occ_scale=occ_scale,
            alpha=alpha,
    ...
    def training_step(self, batch, batch_idx):
        rendered_results = self.model(
            batch, self.randomized, self.white_bkgd, self.near, self.far, mode = 'train',global_step=self.trainer.global_step
        )
        ...
        rgb_coarse_light, rgb_fine_light = rendered_results[0][5], rendered_results[1][5] # normal light RGB
        occ_coarse, occ_fine = rendered_results[0][6], rendered_results[1][6]
        ...
        loss_occ_center = torch.mean((occ_coarse - 0.5) ** 2) + torch.mean((occ_fine - 0.5) ** 2) 
        loss_occ_smooth = (
            torch.mean(torch.abs(occ_coarse[:, 1:, :] - occ_coarse[:, :-1, :])) +
            torch.mean(torch.abs(occ_fine[:, 1:, :] - occ_fine[:, :-1, :]))
         )

        
        
        #loss = loss1 + loss0  + 1e-3*loss_control + 1e-3*loss_structure + 1e-8*loss_cc +self.enhance_weight * loss_enhance    # dark
        loss = loss1 + loss0  + 1e-3*loss_control + 1e-3*loss_structure + 1e-8*loss_cc + 1e-4 * loss_occ_center + 1e-3 * loss_occ_smooth
        ...
        self.log("train/loss_occ_center", 1e-4 * loss_occ_center, on_step=True) 
        self.log("train/loss_occ_smooth", 1e-3 * loss_occ_smooth, on_step=True) 
        self.log("train/occ_mean_coarse", occ_coarse.mean(), on_step=True) 
        self.log("train/occ_mean_fine", occ_fine.mean(), on_step=True)
